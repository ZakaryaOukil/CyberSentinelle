...
const LiveMonitorPage = () => {
  ...
  const startAttackSimulation = async () => {
    ...
    // Send rapid requests to simulate attack
    const sendPings = async () => {
      const promises = [];
      for (let i = 0; i < attackIntensity; i++) {
        promises.push(
          axios.post(`${API}/monitor/ping`).catch(() => {})
        );
      }
      await Promise.all(promises);
    };

    // Continue attack for duration
  ...
  const trafficChartContent = (
    <ResponsiveContainer width="100%" height="100%">
      <AreaChart data={trafficHistory} margin={{ bottom: 30 }}>
        ...
        <Tooltip contentStyle={{ backgroundColor: '#0f172a', border: '1px solid #334155', borderRadius: '8px' }} />
        <Legend />
        <Area type="monotone" dataKey="requests" name="Requêtes" stroke={isUnderAttack ? COLORS.danger : COLORS.primary} fill={isUnderAttack ? COLORS.danger : COLORS.primary} fillOpacity={0.3} />
        <Line type="monotone" dataKey="threshold" name="Seuil d'alerte" stroke={COLORS.warning} strokeDasharray="5 5" dot={false} />
      </AreaChart>
    </ResponsiveContainer>
  );
  ...
  return (
    <div className="space-y-6 fade-in" data-testid="monitor-page">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold font-mono flex items-center gap-3">
            <Radio className={`w-6 h-6 ${isUnderAttack ? 'text-red-500 animate-pulse' : 'text-emerald-500'}`} />
      ...
      {/* Status Banner */}
      <div className={`p-6 rounded-xl border-2 ${isUnderAttack ? 'bg-red-500/20 border-red-500 animate-pulse' : 'bg-emerald-500/20 border-emerald-500'}`}>
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className={`text-2xl font-bold font-mono ${isUnderAttack ? 'text-red-500' : 'text-emerald-500'}`}>
      ...
      {/* Stats Grid */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className={`cyber-card ${isUnderAttack ? 'border-red-500/50' : ''}`}>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-muted-foreground">Requêtes/sec</p>
                <p className={`text-2xl font-bold font-mono ${rps > threshold ? 'text-red-500' : 'text-emerald-500'}`}>
                  {rps}
                </p>
              </div>
              <Zap className={`w-8 h-8 ${rps > threshold ? 'text-red-500' : 'text-emerald-500'}`} />
            </div>
          </CardContent>
        </Card>
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={trafficData?.traffic_history || []}>
                  ...
                  <Tooltip contentStyle={{ backgroundColor: '#0f172a', border: '1px solid #334155', borderRadius: '8px' }} />
                  <Area type="monotone" dataKey="requests" name="Requêtes" stroke={isUnderAttack ? COLORS.danger : COLORS.primary} fill={isUnderAttack ? COLORS.danger : COLORS.primary} fillOpacity={0.3} />
                  <Line type="monotone" dataKey="threshold" name="Seuil" stroke={COLORS.warning} strokeDasharray="5 5" dot={false} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </ClickableChart>
        ...
        <Card className="cyber-card border-red-500/50">
          ...
          <CardContent>
            <div className="flex flex-wrap gap-2">
              {trafficData.attack_indicators.patterns.map((pattern, idx) => (
