...
function AttackParticles({ count = 500 }) {
  ...
  const particles = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      p.push({
        ...
        scale: isAttack ? 0.04 + Math.random() * 0.03 : 0.02 + Math.random() * 0.02,
      });
    }
    return p;
  }, [count]);
  ...
  useFrame((state) => {
    ...
    particles.forEach((particle, i) => {
      ...
      dummy.position.set(
        position[0] + Math.sin(t * speed + offset) * 0.3,
        position[1] + Math.cos(t * speed + offset) * 0.3,
        position[2] + Math.sin(t * speed * 0.7 + offset) * 0.2
      );
      dummy.scale.setScalar(scale * (1 + Math.sin(t * 2 + offset) * 0.3));
      dummy.updateMatrix();
...
function NormalTrafficRing() {
  ...
  const particles = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      p.push({ angle, radius, y: (Math.random() - 0.5) * 0.8, speed: 0.3 + Math.random() * 0.3 });
    }
    return p;
  }, []);
  ...
  return (
    <>
      ...
      <mesh ref={ring3} rotation={[0, Math.PI / 4, 0]}>
        ...
        <meshBasicMaterial color="#FAFF00" transparent opacity={0.2} />
      </mesh>
    </>
  );
}
